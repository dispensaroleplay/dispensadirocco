...
const DELETE_AUDIT_CURSOR_KEY = "discord_delete_audit_cursor";
const DISCORD_AUDIT_MESSAGE_DELETE = 72;

function deleteLogChannelId(env) {
  return String(
    env.DISCORD_DELETE_LOG_CHANNEL_ID || "1540326524484583465"
  ).trim();
}

async function postBotChannelMessage(env, channelId, payload) {
  const botToken = String(env.DISCORD_BOT_TOKEN || "").trim();
  if (!botToken || !/^\d+$/.test(channelId)) {
    throw new Error("DISCORD_BOT_TOKEN or channel id missing for delete logs");
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Delete-log post failed (${response.status}): ${data.message || "unknown"}`
    );
  }
  return data;
}
...
/**
 * Logs Cloudflare-only via audit logs Discord (pas de contenu message).
 * Cron toutes les 5 min.
 */
async function runMessageDeleteAuditLogs(env) {
  const botToken = String(env.DISCORD_BOT_TOKEN || "").trim();
  const guildId = String(env.DISCORD_GUILD_ID || "").trim();
  const logChannelId = deleteLogChannelId(env);

  if (!botToken || !guildId || !env.SITE_STATE) {
    console.warn("[delete-logs] bot token, guild id or SITE_STATE missing — skip");
    return { ok: false, skipped: true };
  }

  const endpoint =
    `https://discord.com/api/v10/guilds/${guildId}/audit-logs` +
    `?action_type=${DISCORD_AUDIT_MESSAGE_DELETE}&limit=30`;
  ...
  if (!response.ok) {
    throw new Error(
      `Audit logs failed (${response.status}): ${data.message || "unknown"}`
    );
  }

  const entries = Array.isArray(data.audit_log_entries)
  ...
  // Plus récent en premier côté Discord — on trie par id croissant pour traiter dans l’ordre.
  const sorted = [...entries].sort((a, b) =>
    BigInt(a.id) < BigInt(b.id) ? -1 : BigInt(a.id) > BigInt(b.id) ? 1 : 0
  );

  const cursor = await env.SITE_STATE.get(DELETE_AUDIT_CURSOR_KEY);
  if (!cursor) {
    const newest = sorted[sorted.length - 1]?.id;
    if (newest) {
      await env.SITE_STATE.put(DELETE_AUDIT_CURSOR_KEY, newest);
    }
    console.log("[delete-logs] cursor initialisé — pas de flood historique");
    return { ok: true, posted: 0, initialized: true };
  }

  const fresh = sorted.filter((entry) => BigInt(entry.id) > BigInt(cursor));
  if (!fresh.length) {
    return { ok: true, posted: 0 };
  }
  ...
  for (const entry of fresh) {
    ...
    const authorId = entry.target_id || null;
    const channelId = entry.options?.channel_id || null;
    const count = Number(entry.options?.count || 1) || 1;

    const lines = [
      "🗑️ **Message(s) supprimé(s)**",
      channelId ? `Salon : <#${channelId}>` : "Salon : *inconnu*",
      authorId ? `Auteur du message : <@${authorId}>` : "Auteur : *inconnu*",
      executorId
        ? `Supprimé par : <@${executorId}>`
        : "Supprimé par : *inconnu*",
      count > 1 ? `Quantité (audit) : **${count}**` : null,
      "_Contenu non disponible (Cloudflare / audit logs Discord)._"
    ].filter(Boolean);

    await postBotChannelMessage(env, logChannelId, {
      content: lines.join("\n"),
  ...
  await env.SITE_STATE.put(DELETE_AUDIT_CURSOR_KEY, lastId);
  console.log(`[delete-logs] ${posted} entrée(s) postée(s)`);
  ...
  async scheduled(event, env, ctx) {
    ctx.waitUntil(
      (async () => {
        try {
          if (event.cron === "*/5 * * * *") {
            await runMessageDeleteAuditLogs(env);
            return;
          }
